...
sz_arr=( 78 1518 ) #指定测试请求的size大小（字节）
TAG=( TCP_RR UDP_RR )
#TAG=(  TCP_RR UDP_RR )
...
do
	echo "${tag}_${threads}" |tee -a $log_dir/${tag}_pack.csv
	echo "PacketSize,THROUGHPUT,Packet(W)/s,MEAN_LATENCY(ms),P99_LATENCY(ms),MAX_LATENCY(ms),Retrans/s,ReransRate" |tee -a $log_dir/${tag}_pack.csv
	...
	do
		...
		echo "${size}B,$THROUGHPUT,${PPS},$MEAN_LATENCY,$P99_LATENCY,$MAX_LATENCY,$Rerans_PPS,${Rerans_Rate}%" |tee -a $log_dir/${tag}_pack.csv
